...
on histone deacetylase inhibitors [23].
...
which treats each spot as a randomized block [43, 36]. Limma implements mixed model methods
...
each individual gene [35, 24]. Moderated t-statistics lead to p-values in the same way that ordinary
...
rate [2]. The adjusted values are often called q-values if the intention is to control or estimate the
...
are useful for ranking genes even in the presence of large deviations from the assumptions [39, 37].
Benjamini and Hochberg’s control of the false discovery rate assumes independence between genes,
although Reiner et al [27] have argued that it works for many forms of dependence as well. The
...
in such details.
The linear model for gene g has residual variance σ2
g with sample value s2
g and degrees of free-
dom dg . The output from lmFit(), fit say, holds the sg in component fit$sigma and the dg in
ˆ
fit$df.residual. The covariance matrix of the estimated
βg is σ2
g CT (XT Vg X)−1C where X is
...
the diagonal elements of CT (XT Vg X)−1C are called unscaled standard deviations and are stored in
ˆ
fit$stdev.unscaled. The ordinary t-statistic for the kth contrast for gene g is tgk=
βgk/(ugksj )
...
The empirical Bayes method assumes a scaled chisquare prior distribution for 1/σ2
g with mean
1/s2
0 and degrees of freedom d0. The posterior values for the residual variances are given by
˜
d0s2
0 + dj s2
s2
...
d0 + dg
62
where dg is the residual degrees of freedom for the gth gene. Mathematically, 1/˜
s2
g is derived as the
posterior mean of 1/σ2
g conditional on s2
g [35, 24]. The output from eBayes() contains s2
0 and d0 as
fit$s2.prior and fit$df.prior and the ˜ s2
j as fit$s2.post. If robust empirical Bayes is used [24],
...
ugk
sg
which can be shown to follow a t-distribution on d0 + dg degrees of freedom if βgk = 0 [35]. By
...
tgk as fit$t with corresponding
...
the expression values for each gene by calling the function lm.wfit. The dispersion model is fitted
to the squared residuals from the mean fit, and is set up to have array specific coefficients, which are
updated in either full REML scoring iterations, or using an efficient gene-by-gene update algorithm.
The final estimates of these array variances are converted to weights which can be used in lmFit.
This method offers a graduated approach to quality assessment by allowing poorer quality arrays,
which would otherwise be discarded, to be included in an analysis but down-weighted.
14.2 Example 1
...
Cancer Centre in Melbourne. This collection of arrays (described in [28]) consists of 100 replicate
...
function. If there are pprint-tip groups across narrays, the model fitting procedure described in [28]
is repeated ptimes to produce a weight for each print-tip group on each array for use in lmFit. This
...
RNA-Seq Data
15.1 Introduction
The limma approach to RNA-seq explained in the articles by Law et al [15], Liu et al [19] and Baldoni
et al [1]. See also the article by Law et al [14], which gives a complete workflow case study. In the
limma approach to RNA-seq, read counts are converted to log2-counts-per-million (logCPM) and
the mean-variance relationship is modeled either with precision weights or with an empirical Bayes
prior trend. The precision weights approach is called “voom” and the prior trend approach is called
“limma-trend” [15]. In either case, the RNA-seq data can be analyzed as if it was microarray data.
This means that any of the linear modeling or gene set testing methods in the limma package can
be applied to RNA-seq data.
...
for this purpose. Other popular methods include RSEM [16] and HTseq. A runnable example
...
method [31] in particular has been found to perform well in comparative studies. This can be applied
...
has been knocked-out in lymphoma cell-lines (GEO series GSE64099) [19]. Overall we obtain more
...
In recent versions of limma and edgeR, we now recommend voomLmFit as the preferred interface for
RNA-seq analyses, replacing voom, lmFit and voomWithQualityWeights. voomLmFit ensures unbaised
estimation of the residual variances, even for genes that have no counts at all in some experimental
conditions [20]. voomLmFit also automates the estimation of sample weights and intrablock correla-
tions. For example, the Schmd1 analysis with sample weights shown in the previous section could
be more efficiently coded by:
> vfit2 <- voomLmFit(x, design=des, sample.weights=TRUE)
> vfit2 <- eBayes(vfit2)
> topTable(vfit2, coef=2, sort.by="P")
...
test for differential splicing associated with any contrast for a linear model [1].
...
The study examined the development of neurons in wild-type and weaver mutant mice [9]. The
...
between within-array duplicates. The theory behind the approach is explained in [37]. This approach
...
> summary(results) E10 E48
-1 40 76
0 12469 12410
1 116 139
> table(E10=results[,1],E48=results[,2]) E48
E10 -1 0 1
-1 29 11 0
0 47 12370 52
1 0 29 87
...
and mature mammary luminal cells. The data was first published by Lim et al [18], who used the
...
to expressed transcript, using the method of Shi et al [32].
> pe <- propexpr(x)
> dim(pe) <- c(4,3)
...
individuals from Ibadan, Nigeria [25] [26]. The profiles were generated as part of the International
...
Get the list of genes belonging to the male-specific region of chromosome Y [34], and a list of genes
...
comparison of males with females, whereas the X genes are significantly down as a group [44]:
...
Brooks et al [3] deposited data for six RNA samples to GEO http://www.ncbi.nlm.nih.gov/geo.
...
can be compared to Supplementary Figure 8A in Brooks et al [3]. The gene trol was identified by
...
146
Bibliography
1. Baldoni, P.L., Chen, L., Li, M., Chen, Y., and Smyth, G.K. (2025). Dividing out quantification
uncertainty enables assessment of differential transcript usage with diffSplice. bioRxiv April
2025, 2025.04.07.647659.
...
4. Callow, M.J., Dudoit, S., Gong, E.L., Speed, T.P., and Rubin, E.M. (2000). Microarray expres-
sion profiling identifies genes with altered expression in HDL deficient mice. Genome Research
10, 2022–2029.
5. Carrel, L. and Willard, H.F. (2005). X-inactivation profile reveals extensive variability in X-
linked gene expression in females. Nature 434, 400–404.
6. Chen, Y., Chen, L., Lun, A.T.L., Baldoni, P.L., and Smyth, G.K. (2025). edgeR v4: powerful
differential analysis of sequencing data with expanded functionality and improved support for
small counts and larger datasets. Nucleic Acids Research 53, gkaf018.
7. Chen, Y., Lun, A.T.L., and Smyth, G.K. (2016). From reads to genes to pathways: differential
...
9. Diaz, E., Ge, Y., Yang, Y.H., Loh, K.C., Serafini, T.A., Okazaki, Y., Hayashizaki, Y., Speed, T.,
...
10. Ellis, L., Pan, Y., Smyth, G.K., George, D., McCormack, C., Williams-Truax, R., Mita, M., Beck,
...
19. Liu, R., Holik, A.Z., Su, S., Jansz, N., Chen, K., Leong, H.S., Blewitt, M.E., Asselin-Labat,
M.L., Smyth, G.K., and Ritchie, M.E. (2015). Why weight? modelling sample and observational
level variability improves power in RNA-seq analyses. Nucleic Acids Research 43, e97.
20. Lun, A.T. and Smyth, G.K. (2017). No counts, no variance: allowing for loss of degrees of
freedom when assessing biological variability from RNA-seq data. Statistical Applications in
Genetics and Molecular Biology 16, 83–93.
21. Milliken, G.A. and Johnson, D.E. (1992). Analysis of Messy Data, Volume 1: Designed Experi-
ments. Chapman & Hall, New York.
22. Oshlack, A., Emslie, D., Corcoran, L.M., and Smyth, G.K. (2007). Normalization of boutique
two-color microarrays with a high proportion of differentially expressed probes. Genome Biology
8, R2.
23. Peart, M., Smyth, G.K., Van Laar, R., Bowtell, D., Richon, V., Marks, P., Holloway, A., and
...
24. Phipson, B., Lee, S., Majewski, I.J., Alexander, W.S., and Smyth, G.K. (2016). Robust hyperpa-
...
25. Pickrell, J.K., Marioni, J.C., Pai, A.A., Degner, J.F., Engelhardt, B.E., Nkadori, E., Veyrieras,
...
148
26. Pickrell, J.K., Pai, A.A., Gilad, Y., and Pritchard, J.K. (2010). Noisy splicing drives mRNA
isoform diversity in human cells. PLoS Genetics 6, e1001236.
27. Reiner, A., Yekutieli, D., and Benjamini, Y. (2003). Identifying differentially expressed genes
using false discovery rate controlling procedures. Bioinformatics 19, 368–375.
28. Ritchie, M., Diyagama, D., Neilson, J., Van Laar, R., Dobrovic, A., Holloway, A., and Smyth,
G.K. (2006). Empirical array quality weights in the analysis of microarray data. BMC Bioinfor-
matics 7, 261.
29. Ritchie, M.E., Phipson, B., Wu, D., Hu, Y., Law, C.W., Shi, W., and Smyth, G.K. (2015). limma
powers differential expression analyses for RNA-sequencing and microarray studies. Nucleic Acids
Research 43, e47.
30. Ritchie, M.E., Silver, J., Oshlack, A., Holmes, M., Diyagama, D., Holloway, A., and Smyth, G.K.
(2007). A comparison of background correction methods for two-colour microarrays. Bioinfor-
matics 23, 2700–2707.
31. Robinson, M.D. and Oshlack, A. (2010). A scaling normalization method for differential expres-
sion analysis of RNA-seq data. Genome Biology 11, R25.
32. Shi, W., De Graaf, C.A., Kinkel, S.A., Achtman, A.H., Baldwin, T., Schofield, L., Scott, H.S.,
Hilton, D.J., and Smyth, G.K. (2010). Estimating the proportion of microarray probes expressed
in an RNA sample. Nucleic Acids Research 38, 2168–2176.
33. Shi, W., Oshlack, A., and Smyth, G.K. (2010). Optimizing the noise versus bias trade-off for
Illumina Whole Genome Expression Beadchips. Nucleic Acids Research 38, e204.
34. Skaletsky, H., Kuroda-Kawaguchi, T., Minx, P.J., Cordum, H.S., Hillier, L., Brown, L.G., Rep-
...
37. Smyth, G.K., Michaud, J., and Scott, H.S. (2005). Use of within-array replicate spots for
assessing differential expression in microarray experiments. Bioinformatics 21, 2067–2075.
38. Smyth, G.K. and Speed, T. (2003). Normalization of cDNA microarray data. Methods 31,
265–273.
39. Smyth, G.K., Yang, Y., and Speed, T. (2003). Statistical issues in cDNA microarray data
analysis. Methods in Molecular Biology 224, 111–136.
149
40. Visvader, J.E. (2009). Keeping abreast of the mammary epithelial hierarchy and breast tumori-
genesis. Genes & development 23, 2563–2577.
41. Wettenhall, J.M., Simpson, K.M., Satterley, K., and Smyth, G.K. (2006). affylmGUI: a graphical
user interface for linear modeling of single channel microarray data. Bioinformatics 22, 897–899.
42. Wettenhall, J.M. and Smyth, G.K. (2004). limmaGUI: a graphical user interface for linear
modeling of microarray data. Bioinformatics 20, 3705–3706.
43. Wolfinger, R.D., Gibson, G., Wolfinger, E.D., Bennett, L., Hamadeh, H., Bushel, P., Afshari, C.,
...
44. Wu, D., Lim, E., Vaillant, F., Asselin-Labat, M., Visvader, J.E., and Smyth, G.K. (2010).
ROAST: rotation gene set tests for complex microarray experiments. Bioinformatics 26, 2176–
2182.
45. Wu, D. and Smyth, G.K. (2012). Camera: a competitive gene set test accounting for inter-gene
correlation. Nucleic Acids Research 40, e133.
46. Yang, Y.H., Dudoit, S., Luu, P., Lin, D.M., Peng, V., Ngai, J., and Speed, T.P. (2002). Normal-
...
47. Yang, Y.H., Dudoit, S., Luu, P., and Speed, T.P. (2001). Normalization for cDNA microarray
...
48. Yang, Y.H. and Speed, T.P. (2003). Design and analysis of comparative microarray experiments.
In T.P. Speed, editor, Statistical Analysis of Gene Expression Microarray Data, pages 35–91.
Chapman & Hall/CRC Press.
49. Yang, Y.H. and Thorne, N.P. (2003). Normalization for two-color cDNA microarray data. In
